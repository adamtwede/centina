...
- Use the `aisl-iterate` project skill (`.claude/skills/aisl-iterate/SKILL.md`) when iterating an `.aisl` file against the checker: run the checker, triage each diagnostic as mechanical-fix-it vs. genuine-ambiguity-ask-the-human, apply the agreed fix, re-run, repeat until clean.
