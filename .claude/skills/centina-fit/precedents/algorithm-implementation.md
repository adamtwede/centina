...
**Verdict:** `partial-fit` — contract only. Centina can state the interface
(what goes in, what shape comes out) and must stay silent on the body, because
the body is exactly the computation — realization, not a relationship.
...
*spec-worthy* lands in the body — the steps — which Centina cannot and should
not express. The signature is writable; the meaningful part is not.
...
unchanged), the recuse/iterate criterion — a spec should describe a
relationship, not encode a computation; algorithmic substance sits outside
Centina's scope.
